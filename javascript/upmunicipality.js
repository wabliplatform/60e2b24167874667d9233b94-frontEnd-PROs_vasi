...
document.getElementById("ikyqv").onclick = event => {
  event.preventDefault();
  const select = document.getElementById("i5f24")
  arrayiichu.push({
      value: select.value,
...
function refreshULi3x25() {
let e=``;
for (let y=0; y<arrayiichu.length; y++)
 {
   e += `<li index='${y}' arrayvalue='${arrayiichu[y].value}'><p style="display: inline-block">${arrayiichu[y].liValue}</p><button class="btn pointer bi bi-trash delete-btn" style="display: inline-block;float: right;background-color: red;color: white;" index='${y}'>&nbsp;Delete</button></li>`;
 }
document.getElementById("i3x25").innerHTML = e;
}

document.getElementById("i3x25").addEventListener("click", event => {
  event.preventDefault();
  arrayiichu = arrayiichu.filter(
    (item, index) => +event.target.getAttribute("index") !== index
  );
  refreshULi3x25();
});
function initializearrayiichu(data) {
  arrayiichu = data.map(item => ({
    value: item._id,
    liValue: item['cname']
  }));
}
document.addEventListener('alignmcandidates', function(e) {
  const advanceSelect = document.getElementById('i5f24');
  ...
  [...advanceSelect.querySelectorAll("[annotationname]")].forEach(
    ...
    );document.getElementById('il64a').onclick = (event) => {
    event.preventDefault();
