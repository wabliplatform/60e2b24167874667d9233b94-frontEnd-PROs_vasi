let apiCandidateApi = new TempApi.CandidateApi();import TempApi from '../src/index';let apiConsultantApi = new TempApi.ConsultantApi();document.getElementById('ifuuql').onclick = (event) => {
    ...
    tableData.forEach((data,index) => {
     ...
     } catch (e) { console.log(e) };window.localStorage.setItem('data', JSON.stringify(Array.from(map.entries())));}});apiConsultantApi.getAllconsultant((error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const subDataElements =[...document.getElementById("i5xbl").querySelectorAll( "[dataitem='true']" )].filter(
    (element, index, array) =>
