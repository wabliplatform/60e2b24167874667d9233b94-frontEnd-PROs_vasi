...
document.addEventListener('alignmcandidates', function(e) {
  ...
  [...advanceSelect.querySelectorAll("[annotationname]")].forEach(
    ...
    let municipalityId = window.location.pathname.replace('/umunicipality/','');let municipality = new TempApi.Municipality();municipality['mimage'] = {
      ...
      document.querySelector('[annotationname = mname]').value = response.body.query.mname ;document.querySelector('[annotationname = mcandidates]').textContent = response.body.query.mcandidates ;{   location.href= '/home' ;}}});};document.getElementById('i0tym').onclick = (event) => {
    event.preventDefault();
    let municipalityId = window.location.pathname.replace('/umunicipality/','');
      if(municipalityId === '/umunicipality' || municipalityId === ''){
        let parentId = "";
        ...
        newMap.forEach((value, key) => {
          if (
            document
              .getElementById(key)
              .contains(document.getElementById("i0tym")) === true &&
    ...
    apiMunicipalityApi.deletemunicipality( municipalityId, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully.');{   location.href= '/home' ;}}});};window.onload = () => {let municipalityId = window.location.pathname.replace('/umunicipality/','');apiMunicipalityApi.getmunicipality( municipalityId, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); const map = new Map();try { document.querySelector('[annotationname = mname]').value = response.body.query.mname; } catch (e) { console.log(e) };try { 
      if(response.body.query.mimage !== undefined){
        if(document.querySelector('[annotationname = mimage]').getAttribute('type') === 'file'){
          document.querySelector('[annotationname = mimage]').setAttribute('data-image-base64',response.body.query.mimage.data);
